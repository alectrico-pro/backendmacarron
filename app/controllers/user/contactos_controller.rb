class User::ContactosController < UsersController
  include Linea
  ...
  def create
    ...
    atributos = contacto_params.except(:return, :__amp_source_origin,:clientId,:rid)
    #El request puede haber sido hecho desde un registro en admin.alectrica.cl, no desde un domino amp cors
    linea.info "Los atributos de contacto son:#{atributos}"
    contacto = User.new(atributos)
    ...
    if contacto.valid? and contacto_params[:clientId]
      ...
      linea.info "El cliente fue exitosamente creado"
      linea.info "Reader #{reader.inspect}"
      linea.info "Cliente #{cliente.inspect}"
      #debiera hacer redirección con success a google para que declare que estoy logado?
      linea.info "Retorno es #{contacto_params[:return]}"
      redirect_to contacto_params[:return] + "#success=true" and return
  ...
  private
    ...
    def contacto_params
      params.permit(:return, :name, :email, :fono, :clientId,:password,:password_confirmation)
    end
